...
with st.sidebar:
    ...
    st.info(f"**Current Mode:** {mode_display[st.session_state.current_mode]}")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("Exam Prep", use_container_width=True):
            st.session_state.current_mode = "exam"
            st.session_state.messages.append({"role": "ai", "content": "📝 **Exam Prep mode activated!** I will help you prepare for tests with practice questions, key concepts, and exam-focused content."})
            st.rerun()
    
    with col2:
        if st.button("Topic Prep", use_container_width=True):
            st.session_state.current_mode = "topic"
            st.session_state.messages.append({"role": "ai", "content": "📚 **Topic Prep mode activated!** I will provide detailed explanations, structured learning, and comprehensive topic coverage."})
            st.rerun()
    
    with col3:
        if st.button("Brush Up", use_container_width=True):
            st.session_state.current_mode = "brushup"
            st.session_state.messages.append({"role": "ai", "content": "⚡ **Brush Up mode activated!** I will give quick reviews, memory aids, and concise summaries for fast revision."})
            st.rerun()
...
def generate_response(user_question, mode, file_content, file_name):
    ...
    mode_prefixes = {
        ...
        "brushup": "⚡ **Brush Up Mode:**\n\n"
    }
    
    mode_instruction = {
        "exam": "\n\n💡 **Exam Tip:** Focus on these concepts for your test!",
        "topic": "\n\n📖 **Learning Tip:** Review these points to master the topic!",
        "brushup": "\n\n🎯 **Quick Recall:** Use these as memory triggers!"
    }
    ...
    if "summar" in question_lower or "summary" in question_lower:
        ...
        if matching_sentences:
            result = f"{prefix}**Key {'Exam' if mode == 'exam' else 'Learning' if mode == 'topic' else 'Review'} Points from '{file_name}':**\n\n"
            limit = 8 if mode == "exam" else 6 if mode == "topic" else 4
            for i, match in enumerate(matching_sentences[:limit], 1):
                if mode == "exam":
            ...
            result = f"{prefix}**Key Points from '{file_name}':**\n\n"
            limit = 8 if mode == "exam" else 6 if mode == "topic" else 4
            for i, s in enumerate(sentences[:limit], 1):
                if mode == "exam":
                    result += f"{i}. 🔑 {s}.\n\n"
                elif mode == "topic":
                    result += f"{i}. 📌 {s}.\n\n"
        ...
        if matching_sentences:
            result = f"{prefix}"
            if mode == "exam":
                result += f"**Exam Explanation from '{file_name}':**\n\n"
            elif mode == "topic":
                result += f"**Detailed Explanation from '{file_name}':**\n\n"
            else:
                result += f"**Quick Explanation from '{file_name}':**\n\n"
            
            limit = 5 if mode == "topic" else 3 if mode == "brushup" else 4
            for i, match in enumerate(matching_sentences[:limit], 1):
                result += f"{i}. {match['sentence']}.\n\n"
            result += instruction
            return result
            ...
            return f"{prefix}No exact match found for: \"{user_question}\"\n\nHere's what your document contains:\n\n\"{preview}...\"\n\nTry asking with different keywords from your document."
    
    else:
        if matching_sentences:
            result = f"{prefix}"
            if mode == "exam":
                result += f"**Answer from '{file_name}' (Exam Focus):**\n\n"
            elif mode == "topic":
                result += f"**Answer from '{file_name}' (Detailed):**\n\n"
            else:
                result += f"**Answer from '{file_name}' (Quick Reference):**\n\n"
